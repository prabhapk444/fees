...
Class Action {
	...
	function save_settings(){
	...
	}function save_course($postData) {
		include 'db_connect.php';
		
		$course_id = isset($postData['id']) ? $postData['id'] : null;
		$course = $postData['course'] ?? '';
		$duedate = $postData['due-date'] ?? '';
		$dueamount = $postData['due-amount'] ?? '';
		...
		if ($course_id) {
			...
			$stmt = $conn->prepare("INSERT INTO courses (course, duedate, dueamount, total_amount, level, description) VALUES (?, ?, ?, ?, ?, ?)");
	...
	function save_student(){
		...
		if(empty($id)){
			...
			$save = $this->db->query("UPDATE student set $data where id_no = $id_no");
		}
		if($save)
			return 1;
	}
	...
	function delete_payment(){
	 ...
	 function manage_mail(){
		echo " inside manage mail in admin_class";
	 }
}